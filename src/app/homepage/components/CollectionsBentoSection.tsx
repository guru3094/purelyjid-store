...
export default function CollectionsBentoSection() {
  ...
  const redirectToWhatsApp = (collectionName: string) => {
    const phone = "919518770073";
    const message = `Hi PurelyJid, I'm interested in "${collectionName}" collection. Can you share more details?`;
        ...
        <div className="bento-section-title mb-14 flex flex-col md:flex-row md:items-end justify-between gap-6" style={{ opacity: 0 }}>
          <div className="space-y-3">
            ...
            <h2 className="font-display text-5xl md:text-7xl font-bold tracking-tighter text-foreground leading-none">
              Crafted for <br />
              <span className="italic text-muted-foreground">Every Moment.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="bento-grid">
          {collections.map((col) => (
            ...
            >
              ...
              {/* Hover Arrow */}
              <div className="absolute bottom-6 right-6 z-20 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white">
                →
              </div>
